...
fn transform_ty<'tcx>(tcx: TyCtxt<'tcx>, mut ty: Ty<'tcx>, parents: &[Ty<'tcx>], options: TransformTyOptions) -> Ty<'tcx> {
    if parents.contains(&ty) {
        //FIXME do we want to have a placeholder based on index instead?
        return tcx.normalize_erasing_regions(ty::ParamEnv::reveal_all(), ty);
    }
    ...
    match ty.kind() {
        ...
        ty::Tuple(tys) => {
            ty = Ty::new_tup_from_iter(tcx, tys.iter().map(|ty| transform_ty(tcx, ty, &parents, options)));
        }
        ...
        ty::Adt(adt_def, args) => {
            ...
            } else if options.contains(TransformTyOptions::GENERALIZE_REPR_C) && adt_def.repr().c()
            {
                ...
                if let Some(field) = field {
                    ...
                    // FIXME this doesn't break cycles, consider
                    // struct X<T> {
                    //   x: u8,
                    //   p: PhantomData<T>,
                    // }
                    ...
                    // #[repr(transparent)]
                    // struct Y(X<Y>);
                    // This may sound unlikely, but this pattern is generated by
                    // self_cell, so it's actually pretty real-world.
                    // In that case, what will happen is that Y will be transformed
                    // to X as it is transparent, and X will try to transform args on Y,
                    // recursing.
                    parents.push(ty0);
        ...
        ty::CoroutineClosure(def_id, args) => {
            ty = Ty::new_coroutine_closure(tcx, *def_id, transform_args(tcx, args, &parents, options));
        }
