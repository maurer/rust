...
fn transform_ty<'tcx>(
    ...
    match ty.kind() {
        ty::Float(..) | ty::Str | ty::Never | ty::Foreign(..) | ty::GeneratorWitness(..) => {}
        ...
        ty::Generator(def_id, args, movability) => {
            ty = Ty::new_generator(tcx, *def_id, transform_args(tcx, args, parents, options), *movability);
        }
...
pub fn typeid_for_instance<'tcx>(
    ...
    if !options.contains(EncodeTyOptions::NO_SELF_TYPE_ERASURE) {
            ...
            && let Some(trait_ref) = tcx.impl_trait_ref(impl_id)
        {
        ...
        } else if tcx.is_closure(instance.def_id()) {
            ...
            let (trait_id, inputs) = match closure_ty.kind() {
                ty::Closure(..) => {
                    ...
                    let trait_id = match closure_args.kind() {
                    ...
                    let tuple_args =
                        tcx.replace_late_bound_regions(closure_args.sig(), |_| tcx.lifetimes.re_erased).0.inputs()[0];
                    (trait_id, tuple_args)
                }
