...
impl<'tcx> Instance<'tcx> {
    ...
    pub fn ty(&self, tcx: TyCtxt<'tcx>, param_env: ty::ParamEnv<'tcx>) -> Ty<'tcx> {
        let poly_ty = tcx.type_of(self.def.def_id());
        let ty = tcx.instantiate_and_normalize_erasing_regions(self.args, param_env, poly_ty);
        if let InstanceDef::CfiShim { invoke_ty, .. } = self.def {
            // Rewrite the signature to use our invoke_ty as Self rather than the real one.
            let rewrite_args = tcx.mk_args_trait(invoke_ty, (*self.args).into_iter().skip(1));
            let rewritten_ty =
                tcx.instantiate_and_normalize_erasing_regions(rewrite_args, param_env, poly_ty);
            // FIXME(maurer) If we generate different vtables based on non-principal predicates
            // when shimming is enabled, we can drop this call and just return rewritten_ty. This
            // will also yield finer grained CFI, at the potential cost of more vtables.
            merge_sigs_for_cfi_shim(tcx, ty, rewritten_ty)
        } else {
            ty
        }
    }
...
impl<'tcx> Instance<'tcx> {
    ...
    pub fn cfi_shim(mut self, tcx: TyCtxt<'tcx>, invoke_ty: Ty<'tcx>) -> ty::Instance<'tcx> {
        if tcx.sess.cfi_shims() {
            if tcx.is_closure_like(self.def.def_id()) {
                ...
                let fn_trait = pep.principal_def_id().unwrap_or_else(|| {
                ...
                // FIXME debug assert that it's the only one
                let method_id = tcx
...
/// Creates a type using the first argument from `rewritten_ty` and all others from `orig_ty`.
/// `orig_ty` and `rewritten_ty` are expected to both be fully instantiated already.
fn merge_sigs_for_cfi_shim<'tcx>(
    tcx: TyCtxt<'tcx>,
    orig_ty: Ty<'tcx>,
    rewritten_ty: Ty<'tcx>,
) -> Ty<'tcx> {
    // The rewritten type will not be able to correctly resolve associated types in the signature.
    // This would not be relevant for a object-safe trait without additional constraints, but there
    // isn't a different vtable for `dyn Foo` and `dyn Foo<AssocTy = Bar>`. This means that:
    // 1. Our `invoke_ty` will be `dyn Foo` in that case, not `dyn Foo<AssocTy = Bar>`
    // 2. Callers who believe they have a `dyn Foo` should not have access to any methods which
    //    contain `AssocTy` in the signature, so they should not be confused by the extra precision
    //    on the type signature.
    // 3. Callers who believe they have a `dyn Foo<AssocTy = Bar>` will need to truncate that
    //    predicate off (leaving only the principal trait) during a CFI check.
    //
    // To compute a type signature for the method which is usable by a `dyn Foo<AssocTy = Bar>`, we
    // take the receiver computed by instantiating at `dyn Foo`, and the rest of the signature
    // computed at our real self type.
    let rewritten_receiver = rewritten_ty.fn_sig(tcx).input(0).skip_binder();

    let rewritten_sig = orig_ty.fn_sig(tcx).map_bound(|orig_sig| {
        let inputs =
            iter::once(rewritten_receiver).chain(orig_sig.inputs().into_iter().skip(1).copied());
    ...
    Ty::new_fn_ptr(tcx, rewritten_sig)
}
